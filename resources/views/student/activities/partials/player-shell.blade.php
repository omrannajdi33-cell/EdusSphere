...
<div
    ...
    data-save-url="{{ $saveUrlOverride ?? (($previewMode || $correctionMode || $isLocked) ? '' : route('student.activities.save', $activity)) }}"
    data-submit-url="{{ $submitUrlOverride ?? ($canSubmit ? route('student.activities.submit', $activity) : '') }}"
    data-correction-url="{{ $correctionMode && $student ? route('admin.activities.corrections.save', [$activity, $student]) : '' }}"
    ...
    data-home-url="{{ route('student.dashboard') }}"
    role="application"
