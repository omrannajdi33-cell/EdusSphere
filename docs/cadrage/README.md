# Index — Documentation de cadrage (Phase 0)

Phase 0 **terminée** le 21 juin 2026.

| Fichier | Description |
|---------|-------------|
| [01-decisions.md](01-decisions.md) | Décisions de base validées (stack, cible, Docker, GitHub) |
| [02-v1-v2-scope.md](02-v1-v2-scope.md) | Périmètre V1 minimale vs report V2 |
| [03-ecrans.md](03-ecrans.md) | 48 écrans Blade (admin + élève + public) |
| [04-routes-laravel.md](04-routes-laravel.md) | ~87 routes Laravel |
| [05-base-de-donnees.md](05-base-de-donnees.md) | 23 tables MySQL avec relations |

Retour : [README principal](../../README.md)
