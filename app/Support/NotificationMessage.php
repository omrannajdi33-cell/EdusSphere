...
class NotificationMessage
{
    ...
    public static function body(string $type, array $data): string
    {
        ...
        return match ($type) {
            ...
            'project_submitted' => ($data['student_name'] ?? 'Un élève').' a soumis le projet « '.($data['project_title'] ?? '').' »',
            'project_corrected' => '« '.($data['project_title'] ?? 'Ton projet').' » a été corrigé'.self::scoreSuffix($data),
            'project_returned' => '« '.($data['project_title'] ?? 'Ton projet').' » a été renvoyé — tu peux le modifier',
